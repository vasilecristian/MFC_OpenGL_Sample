...
namespace gl
{
    GLuint BasicGraphics::m_shaderProgram = 0;
    GLuint BasicGraphics::m_positionAttribLoc = 0;
    GLuint BasicGraphics::m_normalAttribLoc = 0;
    GLuint BasicGraphics::m_colourAttribLoc = 0;
    gl::vec3 BasicGraphics::m_colour;
    GLuint BasicGraphics::m_primitive = GL_TRIANGLES;
    ...
    //     is silently ignored.
    void BasicGraphics::ConnectShader(GLuint shaderProgram,
        const char* positionAttribName,
        const char* normalAttribName,
        const char* colourAttribName)
    {
        m_shaderProgram = shaderProgram;
        m_positionAttribLoc = m_normalAttribLoc = m_colourAttribLoc = 0xFFFFFFFF;
        if (positionAttribName != NULL)
            m_positionAttribLoc = glGetAttribLocation(shaderProgram, positionAttribName);
        if (normalAttribName != NULL)
            m_normalAttribLoc = glGetAttribLocation(shaderProgram, normalAttribName);
        if (colourAttribName != NULL)
            m_colourAttribLoc = glGetAttribLocation(shaderProgram, colourAttribName);
    }
    ...
    void BasicGraphics::SetDrawColour(vec3 colour)
    {
        m_colour = colour;
    }
    ...
    //     Local variable colour is set to the incoming colour. It will be used as
    //     the colour for subsequent draw operations.
    void BasicGraphics::SetDrawColour(float r, float g, float b)
    {
        m_colour = vec3(r, g, b);
    }
    ...
    void BasicGraphics::DrawWireSphere(GLfloat radius, GLint slices, GLint stacks)
    {
    ...
    void BasicGraphics::DrawWireCube(GLfloat size)
    {
        m_primitive = GL_LINE_STRIP;
        DrawSolidCube(size);
        m_primitive = GL_TRIANGLES;
    ...
    void BasicGraphics::DrawSolidSphere(GLfloat radius, GLint slices, GLint stacks)
    {
        ...
        if (lastShaderProg != m_shaderProgram && vertsArray != NULL)
        {
            //connect position and normal arrays to shader
            if (m_positionAttribLoc != 0XFFFFFFFF)
            {
                glEnableVertexAttribArray(m_positionAttribLoc);
                glVertexAttribPointer(m_positionAttribLoc, 4, GL_FLOAT, GL_FALSE, 0,
                    BUFFER_OFFSET(0));
            }

            if (m_normalAttribLoc != 0xFFFFFFFF)
            {
                glEnableVertexAttribArray(m_normalAttribLoc);
                glVertexAttribPointer(m_normalAttribLoc, 4, GL_FLOAT, GL_FALSE, 0,
                    BUFFER_OFFSET(6 * (lastSlices * lastStacks - lastSlices) * 4 * sizeof(GLfloat)));
            }

            if (m_colourAttribLoc != 0xFFFFFFFF)
            {
                //set a constant colour
                glDisableVertexAttribArray(m_colourAttribLoc);
                glVertexAttrib4fv(m_colourAttribLoc, m_colour);
            }

        }
        lastShaderProg = m_shaderProgram;
        ...
        glBindVertexArray(sphere);
        glVertexAttrib4fv(m_colourAttribLoc, m_colour);
        glDrawArrays(m_primitive, 0, sphereVerts);
    }
    ...
    void BasicGraphics::DrawSolidCube(GLfloat size)
    {
        ...
        //If the shader changed and we've drawn before,
        //we need to detach from old shader and connect to new.
        if (lastShaderProg != m_shaderProgram && lastSize != 0)
        {
            //connect position and normal arrays to shader
            if (m_positionAttribLoc != 0XFFFFFFFF)
            {
    ...
    //     array object that are local to this function.
    void BasicGraphics::DrawQuad(vec4 v1, vec4 v2, vec4 v3, vec4 v4)
    {
        ...
        if (lastShaderProg != m_shaderProgram)
        {
            //connect position and normal arrays to shader
            if (m_positionAttribLoc != 0XFFFFFFFF)
            {
        ...
        glVertexAttrib4fv(m_colourAttribLoc, m_colour);
        glVertexAttrib4fv(m_normalAttribLoc, normal);
    ...
    void BasicGraphics::DrawSolidTorus(GLfloat innerRadius, GLfloat outerRadius, GLint nSides, GLint nRings)
    {
        ...
        //If the shader changed and we've drawn before,
        //we need to detach from old shader and connect to new.
        if (lastShaderProg != m_shaderProgram && vertex != NULL)
        {
            //connect position and normal arrays to shader
            if (m_positionAttribLoc != 0XFFFFFFFF)
            {
        ...
        if (nSides != lastSides || nRings != lastRings || iradius != lastIradius || oradius != lastOradius)
        {
            ...
            {
                glEnableVertexAttribArray(m_positionAttribLoc);
                glVertexAttribPointer(m_positionAttribLoc, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
            }

            if (m_normalAttribLoc != 0xFFFFFFFF)
            {
                glEnableVertexAttribArray(m_normalAttribLoc);
                glVertexAttribPointer(m_normalAttribLoc, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(vec4) * triVerts));
        ...
        glBindVertexArray(torus);
        glVertexAttrib4fv(m_colourAttribLoc, m_colour);
        glDrawArrays(m_primitive, 0, verts);
    }

    void BasicGraphics::DrawWireTorus(GLfloat innerRadius, GLfloat outerRadius, GLint nSides, GLint nRings)
    {
        m_primitive = GL_LINE_STRIP;
